...
int getpinfo(struct pstat* procstat)
{
  acquire(&ptable.lock);
  struct proc* p;
  int process_counter = 0; //Keeps track of number of processes in procstat
  for(p = ptable.proc; p != &(ptable.proc[NPROC]); p++) {
    ...
    if(p->state != UNUSED) {
        ...
        procstat->counters[index] = p->counters;
    }
  }

  for(int index = 0; index < process_counter;index++)
  {
    cprintf("Process ID: %d \nProcess Ticks: %d \nProcess Tickets: %d \nProcess Inuse: %d\n", procstat->pid[index], procstat->ticks[index], procstat->tickets[index], procstat->inuse[index] );
  }
  release(&ptable.lock);
  return 0;
}
...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  enQueue1(np);
  //Giving new process same tickets as parent process.
  settickets(np->pid, curproc->tickets);
  cprintf("Forked Process given %d tickets...\n", np->tickets);
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Starting loop to find lottery winner.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
      tickets_passed += p->tickets;
      // This is a greater than equal to symbol since the winner is the process where the ticket we have selected falls in
      // This can only happen if the number of tickets passed is greater than or equal to the winning ticket
      // If it is equal to the winning ticket, then whatever process had that ticket (it assume the ticket was the last one assigned to the process)
      // If it is greater than the number of tickets than the last process whose tickets have been added to the ticket count is the winner process
      if(tickets_passed>=winner){
        // process_found = 1;
        // We do not need the above code since we moved the process running and context switching to the if statement
        c->proc = p;
        switchuvm(p);
        p->state = RUNNING;
        p->inuse = 1; // For PSTAT
